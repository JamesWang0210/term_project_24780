...
void player::knife_position()
{
	...
	else
	{
		if (downPressed == FALSE)
		{
			...
			//right arm setup
			double ratio = 0.4;   // need to check integer input;
			knife.x_lefthand_joint = StickyManOrigin.x - (SC.X_LeftHandJoint_length + SC.X_LeftHandJoint_length2)*ratio + 1; // store the number in struct
			knife.y_lefthand_joint = StickyManOrigin.y + rad + (SC.X_LeftHandJoint_length + SC.X_LeftHandJoint_length2)*ratio + 4;
			//right upper arm
			glVertex2i(StickyManOrigin.x, StickyManOrigin.y + rad);
			glVertex2i(knife.x_lefthand_joint, knife.y_lefthand_joint);

			//right lower arm
			knife.x_lefthand = knife.x_lefthand_joint - (SC.X_LeftHandJoint_length + SC.X_LeftHandJoint_length2)*(1 - ratio - 0.2);
			knife.y_lefthand = knife.y_lefthand_joint - (SC.X_LeftHandJoint_length + SC.X_LeftHandJoint_length2)*(ratio - 0.2); // note;
			...
			//left lower arm
			knife.x_righthand = StickyManOrigin.x + SC.X_RightHandJoint_length - SC.X_RightHandJoint_length2 + 8;
			knife.y_righthand = StickyManOrigin.y + rad + SC.Y_RightHandJoint_length + SC.Y_RightHandJoint_length2;
			glVertex2i(StickyManOrigin.x + SC.X_RightHandJoint_length, StickyManOrigin.y + rad + SC.Y_RightHandJoint_length);
			glVertex2i(knife.x_righthand, knife.y_righthand);
		...
		else
		{
			...
			//right lower arm
			knife.x_lefthand = knife.x_lefthand_joint - (SC.X_LeftHandJoint_length + SC.X_LeftHandJoint_length2)*(1 - ratio - 0.2);
			knife.y_lefthand = knife.y_lefthand_joint - (SC.X_LeftHandJoint_length + SC.X_LeftHandJoint_length2)*(ratio - 0.2);
		...
		knife.x_knife_bottom = knife.x_lefthand + 8;
		knife.y_knife_bottom = knife.y_lefthand + 4;
		knife.x_knife_tip = knife.x_lefthand - 18;     // can use this coordinate to determine knife damange
		knife.y_knife_tip = knife.y_lefthand - 9;
		knife.x_knife_shield_l = knife.x_lefthand + 4;
		knife.y_knife_shield_l = knife.y_lefthand - 5;
		knife.x_knife_shield_r = knife.x_lefthand - 4;
		knife.y_knife_shield_r = knife.y_lefthand + 5;
